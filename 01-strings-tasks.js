...
function getRectangleString(width, height) {
  ...
  const result = first + mid.repeat(height - 2) + last;
  return result;
}
...
function encodeToRot13(str) {
  const a = 'ABCDEFGHIJKLM'.split('');
  const b = 'NOPQRSTUVWXYZ'.split('');
  let string = str.split('');
  string = string.map((el) => {
    const elem = el.toUpperCase();
    let i;
    let char;
    if (a.includes(elem)) {
      i = a.indexOf(elem);
      char = b[i];
    } else if (b.includes(elem)) {
      i = b.indexOf(elem);
      char = a[i];
    } else return el;

    if (el === el.toLowerCase() && (char !== undefined)) {
...
function isString(val) {
  if (typeof val === 'object' && val !== null) {
    if (val.toString().length > 0) return true;
  }
